...
<div class="field col">
	<p>90. Dado o conjunto first a seguir, forneça uma gramática que gera esse conjunto first</p>
	<pre>{`E: {(, id}
X: {+}
X: {ε}
T: {(, id}
...
<hr />
<div class="field col">
	<p>2. Dado o conjunto follow a seguir, forneça uma gramática que gera esse conjunto follow</p>
	<pre>
{`S: {$}
A: {C}
B: {a, C, $}`}</pre>
...
<div class="field col">
	<p>
		6. Mostre os passos da análise sintática ll(1) da string "a b a c" para a seguinte gramática
	</p>
	<pre>{`S -> a A
A -> b S | c`}</pre>
