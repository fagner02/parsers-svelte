	...
	async function follow() {
		try {
			...
			for (let i = 0; i < rules.length; i++) {
				...
				await grammarSelection?.selectFor(`${elemIds.grammar}gset${i}`);
				for (let j = 0; j < rules[i].right.length; j++) {
					...
					if (!nt.includes(symbol)) {
						...
						await selectRSymbol(`${elemIds.grammar}g`, i, j, colors.green, id, false);
						await addPause(id);
						continue;
					}
					await codeCard?.highlightLines([8]);

					await selectRSymbol(`${elemIds.grammar}g`, i, j, colors.blue, id, false);

					if (!$followSet.find((x) => x.left === symbol)) {
						await codeCard?.highlightLines([9]);

						await followSetElement?.addSetRow(symbol, `${elemIds.grammar}gr${i}-${j}`);
						await addPause(id);
					}
					...
					while (true) {
						...
						if (followingSymbol === null) {
							...
							if (!joinSetElement?.has(symbol)) {
								...
								await joinSetElement?.addSetRow(symbol, `${elemIds.grammar}gr${i}-${j}`);
								await addPause(id);
							}
							await codeCard?.highlightLines([15]);
							if (rules[i].left !== symbol) {
								await codeCard?.highlightLines([16]);
								await joinSetElement?.joinSets([rules[i].left], symbol, `${elemIds.grammar}gl${i}`);
								await addPause(id);
							}
						...
						await selectRSymbol(`${elemIds.grammar}g`, i, j + 1, colors.orange, id, false);
						if (nt.includes(followingSymbol)) {
							...
							for (let [key, item] of $firstSet.entries()) {
								...
								if (item.left === followingSymbol) {
									...
									await followSetElement?.joinSets(item.right, symbol, `${elemIds.first}l${key}`);
									await addPause(id);
								}
							}
